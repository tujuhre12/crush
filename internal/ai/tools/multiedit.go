...
- Subsequent edits: normal edit operations on the created content`,
		func(ctx context.Context, params MultiEditParams, call ai.ToolCall) (ai.ToolResponse, error) {
			...
			if len(params.Edits) > 0 && params.Edits[0].OldString == "" {
				response, err = processMultiEditWithCreation(ctx, params, call, permissions, files, workingDir)
			} else {
				response, err = processMultiEditExistingFile(ctx, params, call, permissions, files, workingDir)
			}
...
func processMultiEditWithCreation(ctx context.Context, params MultiEditParams, call ai.ToolCall, permissions permission.Service, files history.Service, workingDir string) (ai.ToolResponse, error) {
	...
	// Check permissions
	_, additions, removals := diff.GenerateDiff("", currentContent, strings.TrimPrefix(params.FilePath, workingDir))
	...
	_, err = files.CreateVersion(ctx, sessionID, params.FilePath, currentContent)
	if err != nil {
...
func processMultiEditExistingFile(ctx context.Context, params MultiEditParams, call ai.ToolCall, permissions permission.Service, files history.Service, workingDir string) (ai.ToolResponse, error) {
	...
	_, additions, removals := diff.GenerateDiff(oldContent, currentContent, strings.TrimPrefix(params.FilePath, workingDir))
	granted := permissions.Request(permission.CreatePermissionRequest{
		SessionID:   sessionID,
		Path:        fsext.PathOrPrefix(params.FilePath, workingDir),
		ToolCallID:  call.ID,
