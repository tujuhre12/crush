...
func (b *McpTool) Run(ctx context.Context, params ai.ToolCall) (ai.ToolResponse, error) {
	...
	if sessionID == "" || messageID == "" {
		return ai.ToolResponse{}, fmt.Errorf("session ID and message ID are required for MCP tool execution")
	}
	...
	mcpToolsOnce.Do(func() {
		...
		for name, m := range cfg.MCP {
			...
			go func(name string, m config.MCPConfig) {
				...
				defer cancel()
				c, err := createAndInitializeClient(mcpCtx, name, m)
				if err != nil {
					return
				}
				mcpClients.Set(name, c)

				tools := getTools(mcpCtx, name, permissions, c, cfg.WorkingDir())
				updateMCPState(name, MCPStateConnected, nil, c, len(tools))
				result.Append(tools...)
			}(name, m)
		}
		wg.Wait()
		mcpTools = slices.Collect(result.Seq())
	})
	return mcpTools
}
